...
#include "erreur_lecture.h"
   ...
   typedef enum {
     ...
     C_FIN_LIGNE,
      ...
      VIRGULE, // '.' separateur pour les nombres decimaux    
      LETTRE,          // lettre alphabetique
      PAR,              // '(' ou ')'
      ACC,              // '{' ou '}'
      POINT_VIRG        // ;
   } Nature_Caractere ;
   ...
   typedef enum { 
      ...
      MUL,              // *
      VIRG,             // ,
      CHAINE,           // plus, moins fois, diviser ou une variable (qui sera appeler CHAINE ici)
                        // que le prof appelle IDF
      DIV,              // /
      MODULO,           // %
      INF,              // <
      SUP,              // >
   ...
   // etait une void function
   resultat_lecture avancer();
